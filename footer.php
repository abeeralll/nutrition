<div class="footer row mt-4 p-5" >
    <div class="d-flex flex-column justify-content-center align-items-center">
        <div class="row">
            <h2>Subscribe Form</h2>
        </div>
        <div class="row">
            <p>Sign up with your email address to receive the most recent scientifically based nutrition research, business updates, recipes and more</p>
        </div>

    </div>
    <div class="d-flex flex-column justify-content-center align-items-center">
        <form class="d-flex flex-wrap">
            <div class="mx-3 my-3">
                <input type="text" class="form-control p-3" placeholder="First Name">
            </div>
            <div class="mx-3 my-3">
                <input type="text" class="form-control p-3" placeholder="Last Name">
            </div>
            <div class="mx-3 my-3">
                <input type="email" class="form-control p-3" placeholder="Email Address">
            </div>
            <button type="submit" class="btn mx-3 my-3" style="background-color:black ;color:#e0b86e;">Submit</button>
        </form>
    </div>
</div>

<style>
    .footer{
    background-color: #e0b86e;
}
</style>